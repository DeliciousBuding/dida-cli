...
| Build Windows amd64/arm64 `dida.exe` | `.github/workflows/release.yml`, release `v0.1.13` assets | done | None. |
| Build Linux amd64/arm64 `dida` | `.github/workflows/release.yml`, release `v0.1.13` assets | done | None. |
| Build Darwin amd64/arm64 `dida` | `.github/workflows/release.yml`, release `v0.1.13` assets | done | Native macOS install smoke is still unavailable. |
| Archive as zip/tar.gz | `.github/workflows/release.yml`, release `v0.1.13` assets | done | None. |
| Generate `checksums.txt` | `.github/workflows/release.yml`, release `v0.1.13` | done | None. |
| Release notes include install methods | `.github/workflows/release.yml` release-notes step | done | None. |
| `install.sh` OS/arch detection and checksum verification | `install.sh` | done | WSL Linux latest smoke passed against `v0.1.13`; macOS native smoke pending. |
| `install.ps1` OS/arch detection and checksum verification | `install.ps1` | done | Windows latest smoke passed against `v0.1.13`. |
...
| npm installer skeleton | `npm/package.json`, `npm/bin/dida`, `npm/scripts/install.js` | partial | Smoke-tested on Windows and WSL Linux against `v0.1.13`; package is not published. |
| Homebrew plan | `docs/distribution.md`, `packaging/homebrew/dida.rb` | partial | Template URL/hash static validation passed against `v0.1.13`; no external tap or native install smoke yet. |
| Scoop plan | `docs/distribution.md`, `packaging/scoop/dida.json` | partial | Template JSON and URL/hash static validation passed against `v0.1.13`; no external bucket or Scoop install smoke on this machine. |
...
- Windows `install.ps1` latest smoke against `v0.1.13`
- WSL Linux `install.sh` latest smoke against `v0.1.13`
- Windows npm installer smoke against `v0.1.13`
- WSL Linux npm installer smoke against `v0.1.13`
- Installed `v0.1.13` binary OpenAPI client config smoke:
...
- Homebrew/Scoop template URL and checksum static validation against
  `v0.1.13/checksums.txt`
- release checksum comparison against `v0.1.13/checksums.txt`
