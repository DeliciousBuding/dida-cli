...
| Official MCP channel is explicit | `docs/research/official-mcp-tool-crosswalk.md`, `docs/research/official-mcp-vs-webapi.md` | done | Live verification needs `DIDA365_TOKEN`. |
| Official MCP high-value wrappers exist | `internal/cli/official_cmd.go`, `docs/research/official-mcp-wrapping-policy.md` | partial | First-class wrappers are implemented, but safe live reads/writes still need a local token and disposable targets. |
...
| Build Windows amd64/arm64 `dida.exe` | `.github/workflows/release.yml`, release `v0.1.4` assets | done | None. |
| Build Linux amd64/arm64 `dida` | `.github/workflows/release.yml`, release `v0.1.4` assets | done | None. |
| Build Darwin amd64/arm64 `dida` | `.github/workflows/release.yml`, release `v0.1.4` assets | done | Native macOS install smoke is still unavailable. |
| Archive as zip/tar.gz | `.github/workflows/release.yml`, release `v0.1.4` assets | done | None. |
| Generate `checksums.txt` | `.github/workflows/release.yml`, release `v0.1.4` | done | None. |
...
- Official MCP live smoke: `DIDA365_TOKEN` is not present in the environment
