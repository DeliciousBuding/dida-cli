...
| Project endpoint family | partially live-verified | `openapi project list` returned a valid list shape; project names, ids, and counts were not committed. Create/update/delete still rely on local dry-run until a disposable project is chosen. | Run `project get/data`, then disposable project writes. |
