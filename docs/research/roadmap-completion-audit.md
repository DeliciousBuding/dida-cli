...
| Release distribution | `.github/workflows/release.yml`, `install.sh`, `install.ps1`, release `v0.1.8` | Implemented and smoke-tested |
...
- Windows installer latest smoke passed against `v0.1.8`.
- WSL Linux installer latest smoke passed against `v0.1.8`.
...
- Package manager templates exist for Homebrew and Scoop under `packaging/`,
  pinned to `v0.1.8` release assets and checksums.
...
- macOS installer smoke should be repeated for `v0.1.8` on a native macOS host.
