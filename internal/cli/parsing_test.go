...
func TestParseTaskListFlags(t *testing.T) {
	cases := []struct {
		...
		{name: "all flags", args: []string{"--filter", "today", "--limit", "5", "--compact"}, filter: "today", limit: 5, compact: true},
		{name: "unknown filter", args: []string{"--filter", "week"}, err: "unknown filter; supported filters: today, all"},
