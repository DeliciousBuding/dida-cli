...
func TestWriteSubcommandHelpDoesNotMutateOrRequireAuth(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "official token set", args: []string{"official", "token", "set", "--help"}, want: "dida official token set --token-stdin"},
		...
		{name: "openapi client set", args: []string{"openapi", "client", "set", "--help"}, want: "dida openapi client set --id"},
