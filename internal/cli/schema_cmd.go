...
func didaCommandSchemas() []commandSchema {
	schemas := []commandSchema{
		...
		{ID: "official.call", Title: "Call an official dida365 MCP tool", Resource: "official", Operation: "write", Command: "dida official call <tool-name> --args-json '{...}' --json", HTTP: []string{"POST https://mcp.dida365.com tools/call"}, Status: "stable", AuthRequired: false, Notes: "Requires DIDA365_TOKEN or saved local official token config. This is the generic entrypoint for all 32 official MCP tools."},
		...
		{ID: "official.habit.create", Title: "Create habit through official MCP", Resource: "official-habit", Operation: "write", Command: "dida official habit create --args-json '{...}' --json", HTTP: []string{"POST https://mcp.dida365.com tools/call create_habit"}, Status: "stable", AuthRequired: false, Notes: "Requires DIDA365_TOKEN or saved local official token config. Payload follows the official MCP tool schema."},
		{ID: "official.habit.update", Title: "Update habit through official MCP", Resource: "official-habit", Operation: "write", Command: "dida official habit update <habit-id> --args-json '{...}' --json", HTTP: []string{"POST https://mcp.dida365.com tools/call update_habit"}, Status: "stable", AuthRequired: false, Notes: "Requires DIDA365_TOKEN or saved local official token config. Payload follows the official MCP tool schema."},
		{ID: "official.habit.checkin", Title: "Upsert a habit check-in through official MCP", Resource: "official-habit", Operation: "write", Command: "dida official habit checkin <habit-id> --date YYYY-MM-DD --value 1 --json", HTTP: []string{"POST https://mcp.dida365.com tools/call upsert_habit_checkins"}, Status: "stable", AuthRequired: false, Notes: "Requires DIDA365_TOKEN or saved local official token config."},
		...
		{ID: "official.focus.delete", Title: "Delete focus record through official MCP", Resource: "official-focus", Operation: "delete", Command: "dida official focus delete <focus-id> --type 0|1 --yes --json", HTTP: []string{"POST https://mcp.dida365.com tools/call delete_focus"}, Status: "stable", AuthRequired: false, ConfirmationRequired: true, Notes: "Requires DIDA365_TOKEN or saved local official token config. Use only when the target focus record is known."},
