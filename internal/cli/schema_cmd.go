...
func didaCommandSchemas() []commandSchema {
	schemas := []commandSchema{
		...
		{ID: "doctor", Title: "Check CLI, config, auth, optional endpoint health, and optional update status", Resource: "system", Operation: "read", Command: "dida doctor --verify --check-upgrade --json", HTTP: []string{"GET /batch/check/0 when --verify is set", "GET GitHub Releases latest when --check-upgrade is set"}, Status: "stable", AuthRequired: false, Notes: "Without --verify and --check-upgrade this command is local-only and reports network_check: not_run plus upgrade_check: not_run. Upgrade check failures are advisory."},
		...
		{ID: "completion", Title: "Generate shell completion script", Resource: "system", Operation: "read", Command: "dida completion bash", Status: "stable", AuthRequired: false, Notes: "Local-only script generation for bash, zsh, fish, and powershell. The generated script is shell text and does not support --json."},
		...
		{ID: "task.latest", Title: "List newest active tasks by creation time", Resource: "task", Operation: "read", Command: "dida task latest --limit 10 --project inbox --compact --json", HTTP: []string{"GET /batch/check/0"}, Status: "stable", AuthRequired: true, Compact: true, Notes: "Sorts active tasks by createdTime descending, falls back to modifiedTime and dueDate when creation metadata is absent. --project inbox maps to the real sync inboxId."},
		...
		{ID: "task.create", Title: "Create task", Resource: "task", Operation: "write", Command: "dida task create --project <project-id> --title <title> --dry-run --json", HTTP: []string{"POST /batch/task", "POST /open/v1/task/{taskId}"}, Status: "stable", AuthRequired: true, DryRun: true, Notes: "Normalizes --start/--due/--reminder. With --reminder: Web body then OpenAPI reminders after account identity match."},
		...
		{ID: "attachment.download", Title: "Download an existing task attachment", Resource: "attachment", Operation: "read", Command: "dida attachment download --project <project-id> --task <task-id> --attachment <attachment-id> --output <file> --json", HTTP: []string{"GET /api/v1/attachment/{projectId}/{taskId}/{attachmentId}?action=download"}, Status: "stable", AuthRequired: true, Notes: "Downloads an already-associated task attachment. Refuses to overwrite local files unless --force is provided. Does not create, associate, preview, or delete attachments."},
		...
		{ID: "account.whoami", Title: "Show cached cross-channel identities", Resource: "account", Operation: "read", Command: "dida account whoami --json", HTTP: []string{}, Status: "stable", AuthRequired: false, Notes: "Reads ~/.dida-cli/identity.json only."},
		{ID: "account.verify", Title: "Refresh and compare channel identities", Resource: "account", Operation: "read", Command: "dida account verify --json", HTTP: []string{"GET /user/status", "GET /user/profile", "GET /open/v1/project"}, Status: "stable", AuthRequired: true, Notes: "Binds webapi userId + project fingerprint and openapi project fingerprint; required before multi-channel reminder writes."},
		...
		{ID: "upgrade", Title: "Self-update CLI binary", Resource: "system", Operation: "write", Command: "dida upgrade --json", Status: "stable", AuthRequired: false, DryRun: false, Notes: "Queries GitHub Releases, verifies SHA-256 checksum, and installs the matched binary. Windows schedules replacement after the current process exits. Use --check for version check only."},
	}
